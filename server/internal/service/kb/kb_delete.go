...
// removed when a knowledge base is hard-deleted. The project deliberately does
// not use database foreign keys, so this plan is the explicit cascade contract.
...
// kbDeletionSummary is logged after a successful cascade so deletion bugs are
// visible without turning on SQL tracing.
...
// DeleteKnowledgeBase deletes a knowledge base and all KB-owned data.
func DeleteKnowledgeBase(kbID int64) error {
	...
	if err := database.DB.Transaction(func(tx *gorm.DB) error {
		...
		return fmt.Errorf("delete KB database rows: %w", err)
	}
